...
class TransferFunction(Spectra):
    """Transfer function. Edited from TransFuncAtRedshift with emphasis on at
    redshift removed.
    ...
    def __init__(
        ...
        super().__init__(
            spec_arr, eng=eng, in_eng=in_eng, rs=rs,spec_type=spec_type,
            rebin_eng=rebin_eng
        )
        self.dlnz = dlnz
    ...
    def at_in_eng(self, new_eng, interp_type='val', log_interp=False,
        ...
        ----------
        new_eng : ndarray
            The injection energies or injection energy bin indices at which to
            interpolate.
        interp_type : {'val', 'bin'}
            The type of interpolation. 'bin' uses bin index, while 'val' uses
            the actual injection energies.
        log_interp : bool, optional
            Whether to perform an interpolation over log of the grid values.
            Default is False.
        bounds_error : bool, optional
            See scipy.interpolate.interp1d.
        fill_value : array-like or (array-like, array-like) or "extrapolate",
            optional
            See scipy.interpolate.interp1d.
        ...
        if (
            ...
            raise TypeError('injection energy must be strictly increasing or \
                            decreasing for interpolation.')
    ...
    def at_eng(
        ...
        ----------
        new_eng : ndarray
            The energy abscissa or energy abscissa bin indices at which to
            interpolate.
        interp_type : {'val', 'bin'}
            The type of interpolation. 'bin' uses bin index, while 'val' uses
            the actual injection energies.
    ...
    def at_val(
        ...
        Interpolation is logarithmic. 2D interpolation should be preferred over
        1D interpolation over each abscissa in the interest of accuracy.

        Parameters
        ----------
        new_in_eng : ndarray
            The injection energy abscissa or injection energy bin indices at
    ...
    def plot(
        ...
        ind : int, float, tuple or ndarray, optional.
            Index or injected energy of Spectrum to plot, or a tuple of indices
            or injected energies providing a range of Spectrum to plot, or a
            list of indices or injected energies of Spectrum to plot.
        step : int, optional
    ...
    def sum_specs(self, weight=None):
        ...
        Applies Spectra.sum_specs, but sets `rs` of the output `Spectrum`
        correctly.

        Parameters
        ----------
        weight : ndarray or Spectrum, optional
            The weight in each redshift bin, with weight of 1 for every bin if
            not specified.

        Returns
        -------
        ndarray or Spectrum
            An array or `Spectrum` of weight sums, one for each energy in
            `self.eng`, with length `self.length`.
    ...
    def __call__(self, weight):
        return self.sum_specs(weight)

    def append(self, spec):
        """Appends a new Spectrum.

        Applies Spectra.append, but first checks that the appended spectrum has
        the same redshift, and is correctly ordered.
        ...
        if self.in_eng.size > 0:
            if self.in_eng[-1] > spec.in_eng:
                raise TypeError("new Spectrum has a smaller injection energy \
                                than the current last entry.")
            if self.rs[-1] != spec.rs:
                raise TypeError('redshift of the new Spectrum must be the \
                                same.')

        super().append(spec)

        
class TransferFunctionInterp:
    """Interpolator of an array of TransferFunction.

    Parameters
    ----------
    grid_vals : ndarray (Only 3+2D implemented!)
        Grid point values of the array of transfer functions (including in_eng
        and eng).
    abscs : dict
        Dict for the transfer function abscissas (including eng and in_eng)
    interp_axes : list
    ...
    interpolator : function
        The interpolation function over the abscissa.
    """

    def __init__(self, grid_vals, abscs, interp_axes, spec_type='N',
                 log_interp=True):
        ...
        if self.log_interp:
            self.grid_vals[self.grid_vals <= 0] = 1e-100
            self.in_func = np.log
    ...
    def __call__(self, **kwargs):
        ...
        out_grid_vals = self.out_func(np.squeeze(self.interpolator(coord)))
        
        return TransferFunction(out_grid_vals, spec_type=self.spec_type,
                                eng=self.abscs['eng'],
                                in_eng=self.abscs['in_eng'])