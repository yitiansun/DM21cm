...
class DarkHistoryWrapper:
    ...
    def set_empty_arrays(self, tf_kwargs):
        '''
    ...
    def get_state_arrays(self):
        ...
        # We only access the data once we are finished calculating deposition. We will set 
        # the tf_kwargs to None, which will result in an error if we try to deposit more
        self.tf_kwargs = None
        
        # Return the deposition quantities
        return self.prop_phot_N, self.emit_phot_N, self.dep_box

    def photon_injection(self, spec_object, injection_type = 'bath', weight_box = None):
        if weight_box is None:
            weight_box = np.ones((self.HII_DIM, self.HII_DIM, self.HII_DIM)) # dummy

        if injection_type == 'bath':
            self.prop_phot_N += self.norm*self.phot_prop_tf(in_spec=spec_object.N, sum_result=True, **self.tf_kwargs) # [N / Bavg]

        if injection_type == 'ots':
